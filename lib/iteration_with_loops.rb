...
def find_min_in_nested_arrays(array_of_daily_temperatures)
    ...
    while row_index<array_of_daily_temperatures.count do
      element_index=0
      minimum_element=array_of_daily_temperatures.count
      while element_index<array_of_daily_temperatures[row_index].count do
        if array_of_daily_temperatures[row_index][element_index].class<minimum_element.class
           minimum_element=array_of_daily_temperatures[row_index][element_index]
        end
        element_index+=1
      end
      min_values<< minimum_element
      row_index+=1 
    end
    min_values
    # src will be an In my head Iarray of arrays of integers
